...
export default function App() {
  ...
  return (
    <div className="relative w-full bg-[#09090b] text-zinc-100 selection:bg-amber-500 selection:text-black">
      
      {/* NAVBAR */}
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isScrolled ? 'bg-zinc-950/80 backdrop-blur-md py-4 border-b border-zinc-900 shadow-lg' : 'py-6'}`}>
        <div className="max-w-[1160px] mx-auto px-7 flex items-center justify-between">
          <div className="flex flex-col gap-px">
            ...
            <span className="text-[0.6rem] tracking-[0.2em] uppercase text-amber-500 font-semibold">Marketing & Vendas</span>
          </div>
          <a href="https://wa.me/559981746266" target="_blank" rel="noreferrer"
             className="hidden md:flex items-center gap-2 bg-white text-black px-6 py-2.5 rounded-full text-sm font-medium tracking-wide uppercase hover:bg-zinc-200 transition-all duration-300">
            Solicitar Orçamento
          </a>
        </div>
      </nav>
      ...
      {/* HERO SECTION */}
      <section ref={heroRef} className="relative h-[200vh] bg-zinc-950 flex flex-col justify-start">
        <div className="sticky top-0 h-screen w-full flex flex-col justify-center items-center overflow-hidden px-7" style={{ perspective: "1200px" }}>
          ...
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_60%_50%_at_50%_40%,rgba(245,158,11,0.06)_0%,transparent_70%)] pointer-events-none" />
          
          <div className="text-center z-20 max-w-[800px] mb-12 flex flex-col items-center">
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mb-4 inline-block text-[0.68rem] font-semibold tracking-[0.18em] uppercase text-amber-500 bg-amber-500/10 px-4 py-1.5 rounded-full">
            ...
            <motion.h1 initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }} className="font-serif text-3xl md:text-5xl lg:text-[4rem] text-white leading-tight mb-6 tracking-tight max-w-[750px]">
              Marketing estratégico que <em className="italic text-amber-400">transforma</em> presença em vendas.
            </motion.h1>
          ...
          <motion.div
            ...
            className="w-full max-w-[640px] h-[38vh] bg-zinc-900 border border-zinc-800 rounded-2xl relative flex flex-col justify-between p-6 shadow-[0_30px_100px_rgba(245,158,11,0.08)] cursor-pointer select-none"
          >
            {/* 1. SEÇÃO DE STORIES SIMULADA NO TOPO */}
            <div className="flex gap-3 overflow-hidden border-b border-zinc-800/60 pb-3 w-full justify-start mb-2">
              {[
                ...
                <div key={i} className="flex flex-col items-center gap-1 shrink-0">
                  <motion.div 
                    ...
                    className={`w-10 h-10 rounded-full p-[2px] ${story.active ? 'bg-gradient-to-tr from-amber-500 via-red-500 to-purple-600' : 'bg-zinc-700'}`}
                  >
                    <div className="w-full h-full rounded-full bg-zinc-900 flex items-center justify-center text-[10px]">
                      {i === 0 ? '👩‍💻' : '✨'}
                    </div>
                  </motion.div>
                  <span className="text-[9px] text-zinc-500 tracking-wide">{story.label}</span>
                </div>
              ))}
            </div>
            ...
            <div className="text-center flex flex-col items-center justify-center flex-1 relative py-2">
              <AnimatePresence>
                {liked && (
                  <motion.div
                    ...
                    className="absolute text-6xl z-30 pointer-events-none drop-shadow-[0_0_20px_rgba(239,68,68,0.5)]"
                  >
                    ❤️
                  </motion.div>
                )}
              ...
              <h3 className="font-serif text-base text-zinc-200">Posicionamento & Autoridade</h3>
              <p className="text-[11px] text-zinc-400 mt-1 max-w-[340px] leading-relaxed">
                Dê dois cliques em qualquer área deste post para interagir com o conteúdo estratégico.
              </p>
            </div>

            {/* 3. BOTTOM BAR INTERATIVO */}
            <div className="flex justify-between items-center border-t border-zinc-800 pt-3 text-[10px] text-zinc-500 w-full">
              <div className="flex gap-4 items-center">
                <motion.span 
                  animate={liked ? { scale: [1, 1.3, 1] } : {}} 
                  className={`flex items-center gap-1 font-medium ${liked ? 'text-red-500' : ''}`}
                >
              ...
              <span className="text-amber-500/70 font-semibold tracking-wide uppercase text-[8px] bg-amber-500/5 px-2 py-0.5 rounded border border-amber-500/10">
                Métricas de Conversão
              </span>
            </div>

            {/* CARDS FLUTUANTES PARALLAX NAS LATERAIS */}
            <motion.div style={{ x: cardLeftX, y: cardLeftY, opacity: cardLeftOpacity }} className="absolute p-4 w-[220px] bg-zinc-950/90 backdrop-blur-md border border-amber-500/20 rounded-xl shadow-2xl z-20 left-[-50px] top-[25%] hidden md:block">
              <div className="text-xs text-amber-500 font-bold mb-1 flex items-center gap-1.5">📈 Conversão Active</div>
              <p className="text-[10px] text-zinc-400 leading-normal">Transformamos interações superficiais em funis de vendas direta para o seu produto ou serviço.</p>
            </motion.div>

            <motion.div style={{ x: cardRightX, y: cardRightY, opacity: cardRightOpacity }} className="absolute p-4 w-[220px] bg-zinc-950/90 backdrop-blur-md border border-blue-500/20 rounded-xl shadow-2xl z-20 right-[-50px] bottom-[20%] hidden md:block">
              <div className="text-xs text-blue-400 font-bold mb-1 flex items-center gap-1.5">📱 Social Reels</div>
              <p className="text-[10px] text-zinc-400 leading-normal">Captação, direção e edição com cortes dinâmicos e roteiro focado em retenção orgânica.</p>
            </motion.div>
          </motion.div>
      ...
      <section className="py-20 bg-zinc-900 border-y border-zinc-800 relative overflow-hidden">
        <div className="max-w-[1160px] mx-auto px-7 relative z-10">
          <motion.div 
            ...
            className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-0 divide-y md:divide-y-0 md:divide-x divide-zinc-800"
          >
            {[
              ...
              <motion.div key={i} variants={cardVariants} className="text-center py-4 md:py-0">
                <div className="font-serif text-4xl md:text-5xl font-bold text-white mb-2">
                  <span className="text-amber-500">{stat.percent ? '' : '+'}</span>
                  {stat.num}
                  {stat.percent && <span className="text-amber-500">%</span>}
                </div>
                <div className="text-[0.75rem] tracking-[0.15em] uppercase text-zinc-400">{stat.label}</div>
              </motion.div>
            ))}
          </motion.div>
        </div>
      ...
      <section ref={textSectionRef} className="py-20 bg-black border-y border-zinc-900 flex flex-col justify-center overflow-hidden">
        <motion.h2 style={{ x: xTextLeft }} className="text-7xl font-black tracking-tight text-zinc-800 uppercase whiteSpace-nowrap select-none m-0">
          SOCIAL MEDIA • BRANDING • STRATEGY • COPYWRITING • FILMMAKING •
        </motion.h2>
        <motion.h2 style={{ x: xTextRight }} className="text-7xl font-black tracking-tight text-amber-500/10 uppercase whiteSpace-nowrap select-none mt-2">
          POSICIONAMENTO PREMIUM • DESIGN DE MARCAS • ESTRATÉGIA COMERCIAL •
        </motion.h2>
      </section>

      {/* DIFFERENTIALS */}
      <section ref={diffTitleRef} className="py-24 bg-zinc-950">
        <div className="max-w-[1160px] mx-auto px-7">
          <div className="text-center mb-16">
            <span className="text-[0.68rem] font-medium tracking-[0.18em] uppercase text-amber-500 bg-amber-500/10 px-4 py-1.5 rounded-full inline-block mb-4">Por que a Isa Melo?</span>
            ...
            <p className="max-w-[550px] mx-auto text-sm font-medium leading-relaxed flex flex-wrap justify-center gap-x-1.5 gap-y-1">
              {words.map((word, i) => {
                ...
                const opacity = useTransform(wordScroll, [start, end], [0.18, 1]);
                return (
      ...
      <section className="py-24 bg-zinc-900 border-t border-zinc-800 text-center relative overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_60%_60%_at_50%_100%,rgba(245,158,11,0.06)_0%,transparent_70%)] pointer-events-none" />
        <div className="max-w-[1160px] mx-auto px-7 relative z-10">
          <motion.div initial={{ opacity: 0, scale: 0.98 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}>
            <span className="text-[0.68rem] font-medium tracking-[0.18em] uppercase text-amber-500 bg-amber-500/10 px-4 py-1.5 rounded-full inline-block mb-6">Comece agora</span>
            <h2 className="font-serif text-3xl md:text-4xl text-white mb-3">Seu posicionamento <em className="italic text-amber-400">começa agora.</em></h2>
            <p className="text-zinc-400 text-sm max-w-[440px] mx-auto mb-10">Vamos construir uma marca forte, estratégica e profissional para o seu negócio.</p>
            
            <a href="https://wa.me/559981746266" target="_blank" rel="noreferrer"
               className="inline-flex items-center gap-3 bg-white text-black px-10 py-4 rounded-full text-sm font-bold uppercase hover:bg-zinc-200 shadow-xl transition-all duration-300"
            >
          ...
          <div className="grid md:grid-cols-2 gap-10 mb-10">
            <div>
              <div className="flex flex-col gap-px mb-4">
                ...
                <span className="text-[0.6rem] tracking-[0.2em] uppercase text-amber-500">Marketing & Vendas</span>
              </div>
              <p className="text-xs text-zinc-400 max-w-[300px] leading-relaxed">Marketing estratégico que transforma presença em vendas. Branding, Social Media e Filmmaker com posicionamento profissional.</p>
            </div>
            <div className="md:justify-self-end">
              <h4 className="font-sans text-[0.72rem] tracking-[0.2em] uppercase text-zinc-500 font-semibold mb-4">Contato</h4>
              <div className="flex flex-col gap-3">
                <a href="https://instagram.com/kaisamelo" target="_blank" rel="noreferrer" className="flex items-center gap-3 text-xs text-zinc-400 hover:text-amber-500 transition-colors">📸 @kaisamelo</a>
                <a href="https://wa.me/559981746266" target="_blank" rel="noreferrer" className="flex items-center gap-3 text-xs text-zinc-400 hover:text-amber-500 transition-colors">💬 (99) 98174-6266</a>
                <a href="mailto:mmelodesignerdemarcas@gmail.com" className="flex items-center gap-3 text-xs text-zinc-400 hover:text-amber-500 transition-colors">✉️ mmelodesignerdemarcas@gmail.com</a>
              </div>
            </div>
          </div>
          <div className="pt-6 border-t border-zinc-900 flex flex-col md:flex-row justify-between gap-4 text-xs text-zinc-500 tracking-wide">
...
function DifferentialCard({ diff, cardVariants }: DifferentialCardProps) {
  ...
  return (
    ...
    >
      <motion.div 
        ...
        style={{
          background: useTransform(
            [mouseX, mouseY],
            ([x, y]) => `radial-gradient(250px circle at ${x}px ${y}px, rgba(245,158,11,0.06), transparent 80%)`
          )
        }}
      />
      ...
      <p className="text-xs text-zinc-400 leading-relaxed relative z-10">{diff.desc}</p>
    </motion.div>
  );
}
    ...
    <section className="py-24 bg-zinc-950 border-t border-zinc-900">
      <div className="max-w-[1160px] mx-auto px-7">
        <div className="text-center mb-14">
          <span className="text-[0.68rem] font-medium tracking-[0.18em] uppercase text-amber-500 bg-amber-500/10 px-4 py-1.5 rounded-full inline-block mb-4">O que oferecemos</span>
          <h2 className="font-serif text-3xl md:text-4xl text-white mb-3">Serviços & Planos</h2>
          <p className="max-w-[520px] mx-auto text-sm text-zinc-400">Soluções estratégicas para construir, fortalecer e posicionar sua marca no digital com resultado real.</p>
        </div>

        {/* TAB CONTROLLERS COM LAYOUTID SLIDE */}
        <div className="flex justify-center mb-14">
          <div className="flex flex-wrap gap-1 bg-zinc-900 border border-zinc-800 p-1.5 rounded-full relative">
            {tabs.map(tab => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-6 py-2.5 relative rounded-full text-[0.82rem] font-semibold tracking-wide uppercase transition-colors duration-300 z-10 ${isTabActive ? 'text-black' : 'text-zinc-400 hover:text-white'}`}
                >
                  {isTabActive && (
                    <motion.div 
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {plans[activeTab].map((plan, i) => (
              <div 
                key={i} 
                className={`relative p-8 rounded-3xl transition-all duration-300 border ${plan.featured ? 'bg-amber-500 text-black shadow-2xl border-amber-500 shadow-amber-500/5' : 'bg-zinc-900 text-zinc-100 border-zinc-800/60 hover:border-zinc-700'}`}
              >
                {plan.badge && (
                  <div className={`absolute -top-3 left-1/2 -translate-x-1/2 text-[0.65rem] font-bold tracking-[0.16em] uppercase px-4 py-1.5 rounded-full whitespace-nowrap ${plan.featured ? 'bg-black text-white' : 'bg-zinc-800 text-amber-500 border border-zinc-700'}`}>
                    {plan.badge}
                  </div>
                )}
                
                <div className={`text-[0.72rem] tracking-[0.18em] uppercase mb-1.5 ${plan.featured ? 'text-black/60 font-semibold' : 'text-zinc-500'}`}>{plan.name}</div>
                
                {plan.customPrice ? (
                  <div className="font-serif text-3xl font-bold italic mb-1.5 text-zinc-100">{plan.price}</div>
                ) : (
                  <div className={`font-serif text-5xl font-bold leading-none mb-1.5 ${plan.featured ? 'text-black' : 'text-white'}`}>
                    <small className="text-lg font-sans font-light mr-1">R$</small>{plan.price}
                  </div>
                )}
                
                <div className={`text-xs mb-6 ${plan.featured ? 'text-black/50' : 'text-zinc-500'}`}>{plan.label}</div>
                <div className={`h-px w-full mb-6 ${plan.featured ? 'bg-black/10' : 'bg-zinc-800'}`}></div>
                
                <ul className="flex flex-col gap-3 mb-8">
                  {plan.features.map((feature, j) => (
                    <li key={j} className="flex items-start gap-2 text-sm">
                      <span className="mt-0.5 shrink-0 text-xs">✔️</span>
                      <span className={plan.featured ? 'text-black/80 font-medium' : 'text-zinc-400'}>{feature}</span>
                    </li>
                  ))}
                </ul>
